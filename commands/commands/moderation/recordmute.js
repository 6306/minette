...
module.exports = {
  ...
  callback: async (message, arguments, text, client) => {
    ...
    let inputID = await message.channel.awaitMessages({filter: user, max: 1, time: 15000, errors: ['time']}).catch(error => {
    ...
    //timeout moment
    let timeoutMessage = await message.channel.send(`Supply a mute duration or time period`)
    let inputTimeout = await message.channel.awaitMessages({filter: user, max: 1, time: 60000, errors: ['time']}).catch(error => {
      message.channel.send('Timeout due to inactivity').then(msg => {
        setTimeout(() => msg.delete(), 10000)
      } )
      return
  ...
  if (typeof inputTimeout == 'undefined') {
    ...
    let inputOffense = await message.channel.awaitMessages({filter: user, max: 1, time: 600000, errors: ['time']}).catch(error => {
      message.channel.send('Timeout due to inactivity').then(msg => {
        setTimeout(() => msg.delete(), 10000)
      } )
      return
  ...
  if (typeof inputOffense == 'undefined') {
    ...
    let inputReason = await message.channel.awaitMessages({filter: user, max: 1, time: 1200000, errors: ['time']}).catch(error => {
      message.channel.send('Timeout due to inactivity').then(msg => {
        setTimeout(() => msg.delete(), 10000)
      } )
      return
