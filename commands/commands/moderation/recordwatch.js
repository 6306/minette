...
module.exports = {
  ...
  callback: async (message, arguments, text, client) => {
    ...
    let inputID = await message.channel.awaitMessages({filter:user, max: 1, time: 10000, errors: ['time']}).catch(error => {
    ...
    //second message
    let offenseMessage = await message.channel.send(`Supply an action taken, if applicable`)
  ...
  if (typeof inputOffense == 'undefined') {
    ...
    let inputReason = await message.channel.awaitMessages({filter: user, max: 1, time: 600000, errors: ['time']}).catch(error => {
      message.channel.send('Timeout due to inactivity').then(msg => {
        setTimeout(() => msg.delete(), 10000)
      } )
      return
